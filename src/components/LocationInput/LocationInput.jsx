...
export default function LocationInput({ icon = "geo", placeholder, setResult, result }) {
    let defaultValue = "";

    if (result) {
        defaultValue = `${result.city.name || ""}${result.city.name && result.city.region ? ", " : ""}${result.city.region || ""}${(result.city.name || result.city.region) && result.city.country ? ", " : ""}${result.city.country || ""}`;
    }
