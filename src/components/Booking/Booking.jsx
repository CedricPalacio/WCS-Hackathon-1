            ...
            .then(function (response) {
                const temporaryResults = response.data.results.map((restaurant) => {
                    ...
                    return {
                        ...
                        location: {
                            ...restaurant.location,
                            formatted_address: restaurant.location.formatted_address.replace(/\\u0026/g, "&").replace(/\\u00e9s/g, "és"),
                        },
                    };
                });
            ...
            .then(function (response) {
                const temporaryResults = response.data.results.map((hotel) => {
                    return {
                        ...
                        location: {
                            ...hotel.location,
                            formatted_address: hotel.location.formatted_address.replace(/\\u0026/g, "&").replace(/\\u00e9s/g, "és"),
                        },
                    };
                });
                ...
                <div id="restaurants-hotels-buttons">
                    ...
                    <button
                        ...
                        onClick={() => {
                            ...
                            document.querySelector(".restaurants-button").classList.add("active");
                            document.querySelector(".hotels-button").classList.remove("active");
                        }}>
                        Restaurants
                    </button>
                    <button
                        ...
                        onClick={() => {
                            ...
                            document.querySelector(".restaurants-button").classList.remove("active");
                        ...
                        <div className="restaurants-list">
                            {restaurantResults.map((restaurant) => (
                                <div
                                    ...
                                    className={`restaurant-displayed ${restaurant.website ? "website-included" : ""}`}
                                    onClick={() => {
                                        restaurant.website && window.open(restaurant.website, "_blank");
                                    }}>
                                    {restaurant.photos && restaurant.photos[0] ? <img src={`${restaurant.photos[0].prefix}300x300${restaurant.photos[0].suffix}`} alt="restaurant" /> : <img src="../../assets/Booking/restaurant.jpeg" alt="restaurant" />}
                                    <div id="restaurant-name-location-rating">
                                        <h3>{restaurant.name}</h3>
                                        <p>{restaurant.location && restaurant.location.formatted_address}</p>
                                        <div id="restaurant-rating-total-ratings">
                                            {restaurant.rating && <StarRatings rating={restaurant.rating / 2} starRatedColor="#0d9488" numberOfStars={5} name="rating" starDimension="20px" starSpacing="1px" />}
                                            {restaurant.stats && (
                                                <p>
                                                    <i className="bi bi-hand-thumbs-up" /> {restaurant.stats.total_ratings}
                                                </p>
                                            )}
                                        </div>
                                        {restaurant.website && (
                    ...
                    <div className="hotels hidden">
                        <div className="hotels-list">
                            {hotelResults.map((hotel) => (
                                <div
                                    key={hotel.fsq_id}
                                    className={`hotel-displayed ${hotel.website ? "website-included" : ""}`}
                                    onClick={() => {
                                        hotel.website && window.open(hotel.website, "_blank");
                                    }}>
                                    {hotel.photos && hotel.photos[0] ? <img src={`${hotel.photos[0].prefix}300x300${hotel.photos[0].suffix}`} alt="hotel" /> : <img src="../../assets/Booking/hotel.jpeg" alt="hotel" />}
                                    <div id="hotel-name-location-rating">
                                        <h3>{hotel.name}</h3>
                                        <p>{hotel.location && hotel.location.formatted_address}</p>
                                        <div id="hotel-rating-total-ratings">
                                            {hotel.rating && <StarRatings rating={hotel.rating / 2} starRatedColor="#0d9488" numberOfStars={5} name="rating" starDimension="20px" starSpacing="1px" />}
                                            {hotel.stats && (
                                                <p>
                                                    <i className="bi bi-hand-thumbs-up" /> {hotel.stats.total_ratings}
                                                </p>
                                            )}
                                        </div>
                                        {hotel.website && (
